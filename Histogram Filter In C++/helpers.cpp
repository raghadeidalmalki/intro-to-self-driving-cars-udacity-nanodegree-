...
vector< vector<float> > normalize(vector< vector <float> > grid) {
    ...
    for (size_t i = 0; i < grid.size(); ++i) {
        newGrid[i].resize(grid[0].size(), 0.0);
    }

    // Calculate the total sum of all the probabilities in the grid
    float total = 0.0;
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
    ...
    newGrid.resize(height);
    for (int i = 0; i < height; ++i) {
        newGrid[i].resize(width, 0.0);
    }

    // Define the blurring factors
    float center_prob = 1.0 - blurring;
    float corner_prob = blurring / 12.0;
    float adjacent_prob = blurring / 6.0;

    // Define the blurring window
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            // Distribute the value to the 3x3 window in newGrid
            for (int dx = -1; dx <= 1; ++dx) {
                for (int dy = -1; dy <= 1; ++dy) {
                    int new_i = (i + dy + height) % height;
                    int new_j = (j + dx + width) % width;
                    newGrid[new_i][new_j] += grid_val * window[dy + 1][dx + 1];
                }
            }
        }
    }
