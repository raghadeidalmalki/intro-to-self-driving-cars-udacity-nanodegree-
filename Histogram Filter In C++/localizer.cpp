...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
    ...
    float total_cells = rows * cols;
    float belief_per_cell = 1.0 / total_cells;

    // Initialize newGrid with the belief_per_cell value
    newGrid.resize(rows);
    for (int i = 0; i < rows; ++i) {
        newGrid[i].resize(cols, belief_per_cell);
    }
